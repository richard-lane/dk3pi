...
{
CombinedCLEOFcn::CombinedCLEOFcn(const std::vector<double>& data,
...
double CombinedCLEOFcn::operator()(const std::vector<double>& parameters) const
{
    ...
    double cleoConstraint = -2*CLEO::cleoLikelihood(_binNumber, params);
    cleoConstraint        = std::isnan(cleoConstraint) ? _nonsenseChi2 : cleoConstraint;
    ...
    return chi2 + cleoConstraint;
}

const double CombinedCLEOFcn::_nonsenseChi2{10000.0};
...
FitResults_t CLEOCombinationFitter::fit(const std::function<double(double)>& efficiency)
{
    ...
    _fitFcn = std::make_unique<CombinedCLEOFcn>(
