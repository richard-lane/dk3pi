...
{
CharmThreshholdFcn::CharmThreshholdFcn(const std::vector<double>& data,
...
double CharmThreshholdFcn::operator()(const std::vector<double>& parameters) const
{
    // The CLEO likelihood may return NaN if Z is outside of the allowed region
    ...
    double charmConstraint = charm_threshhold::chi2(_binNumber, params);
    charmConstraint        = std::isnan(charmConstraint) ? _nonsenseChi2 : charmConstraint;
    ...
    return chi2 + charmConstraint;
}

const double CharmThreshholdFcn::_nonsenseChi2{20000.0};

CharmThreshholdFitter::CharmThreshholdFitter(const std::vector<double>&   binLimits,
...
CharmThreshholdFitter::CharmThreshholdFitter(const std::vector<double>&   binLimits,
                                             ...
                                             const std::array<double, 6>& initialErrors,
                                             const int                    binNumber)
    : CharmThreshholdFitter(binLimits, initialValues, initialErrors, static_cast<BES::Bin>(binNumber))
{
}

FitResults_t CharmThreshholdFitter::fit(const std::function<double(double)>& efficiency)
{
    // This code is exactly the same as for the constrained fitter....
    // TODO refactor to make it nicer
    _fitFcn = std::make_unique<CharmThreshholdFcn>(
